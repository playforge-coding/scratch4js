...
/**
 * Connect scratch blocks with the vm (adapted from scratch-gui's lib/blocks.js;
 * scratch-blocks is bundled directly instead of lazy-loaded).
...
export default function (vm) {
  ...
  const backdropsMenu = function () {
    const next = ScratchBlocks.ScratchMsgs.translate(
      'LOOKS_NEXTBACKDROP',
      'next backdrop',
    );
    const previous = ScratchBlocks.ScratchMsgs.translate(
      'LOOKS_PREVIOUSBACKDROP',
      'previous backdrop',
    );
    const random = ScratchBlocks.ScratchMsgs.translate(
  ...
  const spriteMenu = function () {
    ...
    for (const targetId in vm.runtime.targets) {
      if (!Object.prototype.hasOwnProperty.call(vm.runtime.targets, targetId))
        continue;
  ...
  ScratchBlocks.Blocks.motion_pointtowards_menu.init = function () {
    ...
    const mouse = ScratchBlocks.ScratchMsgs.translate(
  ...
  ScratchBlocks.Blocks.sensing_of_object_menu.init = function () {
    const stage = ScratchBlocks.ScratchMsgs.translate(
      'SENSING_OF_STAGE',
      'Stage',
    );
  ...
  ScratchBlocks.Blocks.sensing_of.init = function () {
    ...
    const menuFn = function () {
      ...
      if (vm.editingTarget) {
        ...
        if (!sensingOfBlock) {
          sensingOfBlock =
            vm.runtime.flyoutBlocks.getBlock(blockId) || defaultSensingOfBlock;
  ...
  ScratchBlocks.Blocks.sensing_touchingobjectmenu.init = function () {
    ...
    const edge = ScratchBlocks.ScratchMsgs.translate(
